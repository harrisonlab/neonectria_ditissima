source("/home/gomeza/git_repos/emr_repos/scripts/neonectria_ditissima/Popgen_analysis/snp/Twisst/plot_twisst.R")
#Set wd for every analysis
setwd("/data/scratch/gomeza/analysis/popgen/topology_noNMaj/contig_13")
########### input data ################
...
window_data_file <- "output.phyml_bionj.w50.data.tsv"
...
pdf(file = paste0("raw.stepped.stacked.pdf"), width = 10, height = 4)
par(mar = c(4,4,1,1))
...
pdf(file = paste0("raw.stepped.unstacked.pdf"), width = 10, height = 4)
par(mar = c(4,4,1,1))
plot.weights(weights_dataframe=weights, positions=cbind(window_data$start,window_data$end),
...
pdf(file = paste0("raw.midpoints.unstacked.pdf"), width = 10, height = 4)
par(mar = c(4,4,1,1))
...
pdf(file = paste0("smooth.stacked.pdf"), width = 10, height = 4)
par(mar = c(4,4,1,1))
...
pdf(file = paste0("smooth.unstacked.pdf"), width = 10, height = 4)
par(mar = c(4,4,1,1))
plot.weights(weights_dataframe=weights_smooth, positions=window_data$mid,
             line_cols=cols, fill_cols=trans_cols, xlim =c(1,1000000),stacked=FALSE)
dev.off()
...
topos = read.tree(file="topologies.trees")

#unrooted trees

pdf(file = paste0("topos.unrooted.pdf"), width = 5, height = 2)
...
pdf(file = paste0("topos.rooted.pdf"), width = 5, height = 2)
par(mfrow = c(1,3), mar = c(1,1,2,1), xpd=NA)
for (n in 1:length(topos)){
