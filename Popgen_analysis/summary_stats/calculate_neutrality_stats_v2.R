...
geom_smooth(aes(y = ThetaW_d[, 1], colour = "Population 1:\nPop1")) +
geom_smooth(aes(y = ThetaW_d[, 2], colour = "Population 2:\nPop2")) +
xlab("Contig coordinate (kbp)") +
ylab(expression(paste("Average Watterson's ", theta, " per site"))) +
labs(colour = "Population") +
scale_colour_manual(values = c("red4", "darkseagreen2")) +
scale_x_continuous(breaks = pretty(xaxis, n = 10)) +
theme(panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), panel.background = element_blank(),
panel.border = element_rect(colour = "black", fill = NA, size = 1),
...
geom_smooth(aes(y = no_seg_d[, 1], colour = "Population 1:\nPop1")) +
